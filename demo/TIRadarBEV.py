...
def radar_BEV_heatmap_generation(adcdata_path, radar_config_path):
    radar_config = load_json(radar_config_path)
    ...
    azimuthFFT_size = radar_config['azimuthFFT_size']
    sig_space = multi_obj_signal_space_mapping(sig_correct, radar_config)
    output_azimuthFFT = np.fft.fftshift(np.fft.fft(sig_space[:, :, 0], n=azimuthFFT_size, axis=1), axes=1)

    heatmap = np.abs(output_azimuthFFT).reshape((
        output_dopplerFFT.shape[0], output_dopplerFFT.shape[1], azimuthFFT_size
    ))
    index_v_zero = heatmap.shape[1] // 2
    heatmap_static = heatmap[:, index_v_zero, :]  #(range=256, azimuth=256)

    index_v_nonzero = list(range(heatmap.shape[1]))
    index_v_nonzero.pop(index_v_zero)
    heatmap_dynamic = heatmap[:, index_v_nonzero, :].sum(axis=1)  #(range=256, azimuth=256)

    return heatmap_static, heatmap_dynamic


if __name__ == '__main__':
    adcdata_path = '0000.adcdata.bin'
    radar_config_path = 'radar.json'
    radar_heatmap = radar_BEV_heatmap_generation(adcdata_path, radar_config_path)
    static = radar_heatmap[1]
    print(static.shape)
    import matplotlib.pyplot as plt
    plt.imshow(static)
    plt.show()
    radar_angle_heatmap_generation(adcdata_path, radar_config_path)