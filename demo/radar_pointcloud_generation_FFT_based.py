...
def radar_pointcloud_generation_FFT_based(args):
    adcdata_path = args.adcdata_path
    ...
    if output_doa is not None:
        pointcloud = generate_pointcloud(output_doa)

    if args.donot_show:
        # show Range-Doppler Curves
        sig_integrate = 10 * np.log10(np.sum(np.power(np.abs(output_dopplerFFT.reshape((output_dopplerFFT.shape[0], output_dopplerFFT.shape[1], -1))), 2), axis=2) + 1)
        # plt.hist(sig_integrate.reshape((-1)))
        # plt.show()

        range_bins = [(i+1)*radar_config['range_resolution'] for i in range(radar_config['rangeFFT_size'])]
        range_bins = np.array(range_bins)
        velocity_bins = [(i-radar_config['dopplerFFT_size']/2)*radar_config['velocity_resolution'] for i in range(radar_config['dopplerFFT_size'])]
        velocity_bins = np.array(velocity_bins)
        detected_point_range_index = output_doa['range_index']
        detected_point_doppler_index = output_doa['doppler_index_origin']

        fig1 = plt.figure(1)
        fig1_ax1 = fig1.add_axes([0.1, 0.1, 0.8, 0.8])
        # fig1_ax1 = fig1.add_axes([0.1, 0.1, 0.6, 0.75])
        for i in range(sig_integrate.shape[1]):
            if i == radar_config['dopplerFFT_size']/2:
                fig1_ax1.plot(range_bins, sig_integrate[:, i], color='black', linewidth=2, label='v={:.2f}'.format(velocity_bins[i]))
            else:
                fig1_ax1.plot(range_bins, sig_integrate[:, i], linewidth=1, label='v={:.2f}'.format(velocity_bins[i]))
        fig1_ax1.scatter(range_bins[detected_point_range_index],
                         sig_integrate[detected_point_range_index, detected_point_doppler_index],
                         s=20, marker='o', c='r')
        # fig1_ax1.legend(bbox_to_anchor=(1.05, 1), loc='upper left', borderaxespad=0.)
        fig1_ax1.grid(True)
        fig1_ax1.set_xlabel('range(m)')
        fig1_ax1.set_ylabel('energy(dB)')
        plt.title('Range-Doppler Curves')

    if args.donot_show:
        # show Range-Doppler Map
        fig2 = plt.figure(2)
        fig2_ax1 = fig2.add_axes([0.1, 0.1, 0.8, 0.8])
        im = fig2_ax1.pcolor(sig_integrate,
                             cmap=plt.cm.jet.with_extremes(under='k', over='w'),
                             norm=colors.Normalize(vmin=sig_integrate.max() * 0.5, vmax=sig_integrate.max()),
                             edgecolors='k', linewidths=0.4)
        plt.xticks(np.arange(sig_integrate.shape[1])+0.5, labels=['{:.2f}'.format(v) for v in velocity_bins],
                   rotation=45, rotation_mode='anchor', ha='right')
        ytick_sample = 4
        plt.yticks(np.arange(0, sig_integrate.shape[0], ytick_sample)+0.5, labels=['{:.2f}'.format(r) for r in range_bins[::ytick_sample]])
        plt.title('Range-Doppler Map')
        fig2.colorbar(im, ax=fig2_ax1, label='intensity(dB)')

    if args.donot_show:
        velocity_bins = [(i - radar_config['dopplerFFT_size'] / 2) * radar_config['velocity_resolution'] for i in
                         range(radar_config['dopplerFFT_size'])]
        velocity_bins = np.array(velocity_bins)

        # show img and pointcloud
        fig3 = plt.figure(3)
        fig3_ax1 = fig3.add_axes([0.1, 0.1, 0.8, 0.8])
        ...
        projected_pixel = np.matmul(TIRadar_to_LeopardCamera1_TransformMatrix,
                                    np.vstack((pointcloud[:, 0:3].T, np.ones((1, pointcloud.shape[0])))))
        projected_pixel = projected_pixel / projected_pixel[2, :]
        projected_pixel = projected_pixel[0:2, :].T
        mask = np.logical_and(np.logical_and(projected_pixel[:, 0] >= 0, projected_pixel[:, 0] <= img.shape[1]),
                              np.logical_and(projected_pixel[:, 1] >= 0, projected_pixel[:, 1] <= img.shape[0]))
        im = fig3_ax1.scatter(projected_pixel[mask, 0], projected_pixel[mask, 1], c=pointcloud[mask, 3], cmap='jet',
                              s=10,
                              norm=colors.Normalize(vmin=velocity_bins.min(), vmax=velocity_bins.max()), marker='o')
        fig3.colorbar(im, ax=fig3_ax1, orientation='horizontal', label='velocity(m/s)')
        # im = fig3_ax1.scatter(projected_pixel[mask, 0], projected_pixel[mask, 1], c=pointcloud[mask, 4], cmap='jet', s=10,
        #                       norm=colors.LogNorm(vmin=pointcloud[:, 4].min(), vmax=pointcloud[:, 4].max()), marker='o')
        # fig3.colorbar(im, ax=fig3_ax1, orientation='horizontal',
        #               label='intensity')
        plt.title('Image and Pointcloud')
    ...
    if args.donot_show:
        ...
        # show img and pointcloud
        fig4 = plt.figure(4)
        fig4_ax1 = fig4.add_axes([0.1, 0.1, 0.8, 0.8])

        im = fig4_ax1.scatter(pointcloud[:, 0], pointcloud[:, 1], c=pointcloud[:, 3], cmap='jet',
                              s=10,
                              norm=colors.Normalize(vmin=velocity_bins.min(), vmax=velocity_bins.max()), marker='o')
        fig4.colorbar(im, ax=fig4_ax1, orientation='horizontal', label='velocity(m/s)')
        fig4_ax1.grid(True)
        fig4_ax1.set_xlabel('x(m)')
        fig4_ax1.set_ylabel('y(m)')
        bin_width = 10
        r_max = np.ceil(range_bins.max() / bin_width) * bin_width
        x_ticks = np.arange(-r_max, r_max + 1, bin_width)
        y_ticks = np.arange(0, r_max + 1, bin_width)
        fig4_ax1.set_xlim(x_ticks.min(), x_ticks.max())
        plt.xticks(x_ticks, labels=['{}'.format(i) for i in x_ticks],
                   rotation=45, rotation_mode='anchor', ha='right')
        fig4_ax1.set_ylim(y_ticks.min(), y_ticks.max())
        plt.yticks(y_ticks, labels=['{}'.format(i) for i in y_ticks])
        fig4_ax1.set_aspect('equal')
        plt.title('Pointcloud BEV')
...
def get_args():
    ...
    parser.add_argument('--adcdata_path', type=str, default='./data_test/scene7/0000.adcdata.bin')
    parser.add_argument('--radar_config_path', type=str, default='./data_test/radar.json')
    parser.add_argument('--donot_show', action='store_false')
    parser.add_argument('--img_path', type=str, default='./data_test/scene7/0000.png')
...
if __name__ == '__main__':
    args = get_args()
    radar_pointcloud_generation_FFT_based(args)
